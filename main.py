...
def main(tickers, indicators, start_date, end_date, interval, data_source, column):
    ...
    if start_date is None:
        while True:
            start_date = input(
                "Please enter start date in the following format - (YYYY-MM-DD):\n"
            )
    ...
    if end_date is None:
        while True:
            end_date = input(
                "Please enter end date in the following format - (YYYY-MM-DD):\n"
            )
    ...
    for ticker in tickers:
        ...
        for name, params in indicators:
            ...
            calculated_series = indicator.calculate(data)
            # calculated_indicators[name] = (calculated_series, params)
            calculated_indicators[f"{name}_{'_'.join(map(str, params))}"] = (
                calculated_series,
                params,
            )
        ...
        try:
            plotter.plot(
                data, calculated_indicators, column=column, company_name=ticker
            )
